...
const checks = [
  ...
  [script.includes("data-hu344-source-boxes") && script.includes("selectedComposition") && script.includes("totalTakenFromPallet"), "Cada pallet origen permite definir cuántas cajas se toman."],
  [script.includes("data-hu344-source-reference") && script.includes("takenComposition"), "Los pallets mixtos permiten tomar cajas por cada referencia."],
  [script.includes("data-hu344-source-reference-toggle") && script.includes("chosen.selected") && css.includes(".hu344-origin-reference"), "Cada referencia de un pallet mixto puede incluirse o excluirse del rearmado."],
  [html.includes("data-hu344-result-list") && !html.includes("data-hu344-add-result") && script.includes("results.length !== 1"), "La vista genera exactamente un pallet resultado."],
  [!script.includes("data-hu344-result-boxes") && script.includes("boxes: total"), "Las cajas del resultado se calculan desde los orígenes y no se duplican en otro campo editable."],
  [script.includes('reference: referenceCodes.length === 1 ? referenceCodes[0] : "MIXED"') && script.includes("references: referenceCodes"), "El único resultado conserva una o varias referencias seleccionadas."],
  ...
  [script.includes("originCount") && script.includes("resultCount: 1"), "El registro conserva relaciones uno-a-uno o muchos-a-uno con un único resultado."],
  ...
  [script.includes("remainingComposition") && script.includes("PALLET_INCOMPLETO_EN_ZE") && script.includes("CONSUMIDO_TOTAL"), "Se conserva el consumo parcial o total de cada pallet origen."],
